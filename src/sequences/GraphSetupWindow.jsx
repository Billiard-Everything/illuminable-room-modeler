...
import { AlertTriangle, Eye, EyeOff, Plus, ScatterChart, Trash2, X } from 'lucide-react';
...
export default function GraphSetupWindow({
  ...
  const handleToggleGraphVisible = (id) => {
    setIsDirty(true);
    onToggleVisible(id);
  };

  const handleGraphColorChange = (id, color) => {
    setIsDirty(true);
    onColorChange(id, color);
  };

  const handleAngleInputChange = (id, key, val) => {
    setIsDirty(true);
    onAngleDraftChange(id, key, val);
  };

  const handleAngleStepInputChange = (id, val) => {
    setIsDirty(true);
    onAngleStepDraftChange(id, val);
  };

  const handleSequenceDraftInputChange = (id, val) => {
    setIsDirty(true);
    ...
    <>
        ...
        >
                  ...
                  >
                    <div className="mb-3 flex items-center gap-2">
                      ...
                      <input
                        type="color"
                        value={row.color}
                        onChange={event => handleGraphColorChange(row.id, event.target.value)}
                      ...
                      <button
                        type="button"
                        onClick={() => handleToggleGraphVisible(row.id)}
                      ...
                      <button
                        type="button"
                        onClick={() => handleRemoveGraph(row.id)}
                    ...
                    <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
                      <label className="block">
                        ...
                        <input
                          ...
                          onChange={event => handleAngleInputChange(row.id, 'a', event.target.value)}
                      ...
                      <label className="block">
                        ...
                        <input
                          ...
                          onFocus={() => onSelect(row.id)}
                          onChange={event => handleAngleInputChange(row.id, 'b', event.target.value)}
                          onKeyDown={event => handleAngleKeyDown(event, row.id)}
                          placeholder="e.g. 50"
                          title="Press Enter to apply, Escape to discard the edit."
                          className="w-full rounded-md border border-white/10 bg-[#080b0f] px-2 py-1.5 font-mono text-xs text-slate-100 outline-none placeholder:text-slate-600 focus:border-cyan-300/60"
                      ...
                      <label className="block">
                        ...
                        <input
                          ...
                          onChange={event => handleAngleStepInputChange(row.id, event.target.value)}
                    ...
                    <label className="mt-3 block">
                      ...
                      <input
                        ...
                        onFocus={() => onSelect(row.id)}
                        onChange={event => handleSequenceDraftInputChange(row.id, event.target.value)}
          ...
          <footer className="flex flex-wrap items-center justify-between gap-2 border-t border-white/10 px-4 py-3">
            ...
            <div className="flex items-center gap-2">
              <button type="button" onClick={handleRequestClose} className="rounded-md border border-white/10 bg-[#0b1016] px-3 py-1.5 text-xs font-bold text-slate-300 transition-colors hover:bg-[#172230]">
                Close
              </button>
              <button
                type="button"
                onClick={() => { setIsDirty(false); onOpenPlot(); }}
                className="flex items-center gap-1.5 rounded-md border border-cyan-300/45 bg-cyan-500/20 px-3 py-1.5 text-xs font-bold text-cyan-100 transition-colors hover:bg-cyan-500/30"
              >
      ...
      {showWarningModal && (
        <div
          ...
          role="presentation"
          onMouseDown={() => setShowWarningModal(false)}
        >
          <div
            className="w-full max-w-md rounded-xl border border-amber-500/30 bg-[#151c24] p-5 shadow-2xl"
            role="dialog"
            aria-modal="true"
            aria-labelledby="unsaved-warning-title"
            onMouseDown={event => event.stopPropagation()}
          >
            <div id="unsaved-warning-title" className="mb-2 flex items-center gap-2.5 text-sm font-bold text-amber-300">
              <AlertTriangle className="h-5 w-5 shrink-0 text-amber-400" />
              Unsaved Graph Changes
            </div>
            <p className="mb-5 text-xs leading-relaxed text-slate-300">
              No changes will take effect unless the <strong className="text-cyan-200">Open / Refresh Plot</strong> button is pressed after a new graph setup.
            </p>
            <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
              <button
                type="button"
                onClick={() => setShowWarningModal(false)}
                className="rounded-md border border-white/10 px-3 py-1.5 text-xs font-bold text-slate-300 transition-colors hover:bg-white/10"
              >
                Cancel
              </button>
              <button
                type="button"
                onClick={onClose}
                className="rounded-md border border-red-500/30 bg-red-500/10 px-3 py-1.5 text-xs font-bold text-red-200 transition-colors hover:bg-red-500/20"
              >
                Close Without Plotting
              </button>
              <button
                type="button"
                onClick={() => {
                ...
                className="flex items-center justify-center gap-1 rounded-md border border-cyan-400/40 bg-cyan-500/20 px-3 py-1.5 text-xs font-bold text-cyan-100 transition-colors hover:bg-cyan-500/30"
              >
